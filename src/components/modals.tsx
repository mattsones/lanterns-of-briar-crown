  ...
  const recruitConversation = (option) => {
    ...
    if (flags[`${option.id}Status`] === "joined") {
      ...
      setDialogue({
        ...
        name: option.name,
        text: `${option.name} gathers ${option.pronouns.possessive} gear and falls back into step beside you.`,
...
Your approach is right, but the words land awkwardly in the noise of the inn. ${option.name} does not walk away. ${option.pronouns.subject === "she" ? "She" : "He"} waits to see whether you mean it plainly.`, choices: [{ label: `Say it plainly and ask ${option.pronouns.object} to come.`, effect: () => join(check) }, { label: "Step back for now.", effect: () => setDialogue(null) }] });
    };
    const goodChoice = recruitmentCheckAttempted
      ? { label: `Ask plainly if ${option.pronouns.subject} will come.`, effect: () => join() }
