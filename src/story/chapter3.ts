...
export const CHAPTER_3_SCENE_COPY = {
  ...
  closing: {
    ...
    text:
      "In the Mossgarden, Noma lays the recovered transfer tag beside an older carved route index and brushes moss from the matching Lower West Gate symbol. The carving has been here for generations; the fresh evidence is the clerk's transfer record.\n\nIt records the unnamed courier entering alive under guard, then leaving through the Lower West Gate bound west. Lume's checkpoint cup identifies that courier as Lio.\n\nMara reads the two records together, then closes both hands around the blue string at her wrist. \"He got one message through. Now we know which way they took him.\"\n\nNoma aligns the transfer notation with Edden's drawing. \"The Lower West Gate opens into the Underway. This notation points toward its keeper-built listening circuit—the Listening Mile. It cannot be followed like an ordinary road map.\"\n\nThe westward passage is still dark, but it is no longer an unnamed direction.",
  },
} as const;
...
export const CHAPTER_3_FULL_SCENE_COPY = {
  ...
  rootbread: {
    ...
    routineResponse: "\"Everybody waiting here gets the tray before the gate decides,\" the child says. \"If the handlers refused it, Bramwell would have asked why they were breaking the ordinary routine. So they let him eat. When one turned toward the account rail, he pulled a short blue thread from his cuff and wound it around the cup before he gave it back.\"",
