...
An August 3 visual follow-up kept the interaction direction accepted but identified a separate presentation gap: the solved construction looked cropped rather than visibly folded, the west fold already appeared to complete the route without the required south fold, and the cache position added unexplained marks. The graybox now distinguishes the folding table from the shrinking paper footprint, outlines and shades reverse-face flaps, strengthens crease treatment, and compacts landed handles. The west fold leaves a deliberate road gap; the south fold supplies a recognizable bridge span and keeper ring; the optional north fold points from that bridge to a separate cache mark; and post-trace feedback explicitly rejects the later office shortcut. Unexplained numeric revision years were removed until the story establishes a usable calendar. Production art remains deferred and must preserve these exact folded relationships. Final validation passed `npm.cmd run build` and all three `npm.cmd run playtest:chapter4` browser tests.
