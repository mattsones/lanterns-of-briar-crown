...
- The selected Great Survey political map now lives at `assets/reference/source-art/assets/maps/great-survey-of-alderreach-v06.png`. Veyrun and Rainroot occupy comparable settled valleys on either side of the braided river march; the long Sunreach escarpment creates waterfall gorges and limited passes to a broad mainland Ember Coast; Coravene, Whiteharbor, Tideglass, Cinder Vale, and Valehaven sit on that mainland while the Saltwake Isles remain a separate offshore chain. National capitals use stars, provincial capitals use diamonds, council seats use hexagons, towns use dots, and Riverwatch uses a fortress square. The approved labels are Hearthward, Lanthorne, Bellwater, Saffron Gate, Kestovar, Cairnmeet, Sevenbridge, Tideglass, Coravene, Whiteharbor, Valehaven, and Riverwatch. Westroot remains a later graphite correction made after renewed contact. V01–V04 are preserved as superseded compositions, and V05 remains preserved as the superseded pre-rename source. V06 changes the painted principality label to `CORAVENE` while preserving the accepted composition.
...
3. After the Chapter 3 compact, unlock or show the selected v06 map with Westroot penciled in and dated under a witness name.
4. Create an optimized runtime derivative or focused crops from `great-survey-of-alderreach-v06.png`; never import the full reference PNG directly.
