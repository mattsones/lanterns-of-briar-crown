...
After the current Chapter 4 enemy production work, run the roster-wide art and combat cohesion review recorded in `docs/planning/enemy-cohesion-pass.md`. It must check both portrait smoothing and whether every enemy's name, appearance, environment, stats, intent labels, and implemented abilities tell the same story.
